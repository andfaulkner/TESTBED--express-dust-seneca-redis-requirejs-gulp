...
(function senecaIndex() {
    ...
    seneca.add({ role: 'math', cmd: 'sum' }, // pattern to match; can be any properties
    function (msg, respond) {
        // action fn: operates on params given in obj w pattern 2 match
    ...
    */
    ...
    .add({ role: 'math', cmd: 'sum', integer: true }, function (msg, respond) {
        this.act({
            role: 'math',
            cmd: 'sum',
    ...
    .add({ role: 'math', cmd: 'product' }, function (msg, respond) {
        var product = msg.left * msg.right;
        respond(null, { answer: product });
    })

    //Note the action pattern: it's defined in more convenient 'jsonic' format
    .add('role:math,cmd:product,integer:true', function (msg, respond) {
        this.act({
            role: 'math',
            cmd: 'product',
            left: Math.floor(msg.left),
    ...
    seneca.act({ role: 'math', cmd: 'sum', left: 1, right: 5 }, consoleCatcher).act({ role: 'math', cmd: 'product', left: 5, right: 8 }, consoleCatcher).act({ role: 'math', cmd: 'sum', integer: true, left: 48, right: 21 }, consoleCatcher);
    //********************************************************************************//
